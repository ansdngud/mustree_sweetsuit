...
def find_keypoint(
    ...
    # 모델 파일의 절대 경로 설정
    model_root = Path("/mnt/nas4/mwh/mustree_sweetsuit/api/sweet_suit")
    pretraind_model = model_root / f"removed_bg_model/{body_position}/{model_position}.pth"

    model = pose_hrnet.get_pose_net(is_train=False).cuda()
    model.load_state_dict(torch.load(str(pretraind_model)), strict=True)
