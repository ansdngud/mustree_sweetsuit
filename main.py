...
def save_file(file: UploadFile, path: Path):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"File save error: {str(e)}")
...
async def sweet_suit(
    ...
    image_front_file: Annotated[
        UploadFile, File(media_type="image/png",
                         description="Front body image")
    ],
    image_side_file: Annotated[
        UploadFile, File(media_type="image/png", description="Side body image")
    ],
    json_front_file: Annotated[
        UploadFile, File(media_type="text/json",
                         description="Front depth data")
    ],
    ...
    - **Request Parameters**:
         - **`upper_or_lower`**: indicate whether it is upper or lower body
            - **Example**:
                upper body : upper / lower body : lower

         - **`image_front_file`**: front body image for measurement
            - **Example**:

                <img src='http://203.252.147.202:8000/static/sweet_suit/manne_upper_front.jpg' width=150 height=200>

        - **`image_side_file`**: side body image for measurement
            - **Example**:

                <img src='http://203.252.147.202:8000/static/sweet_suit/manne_upper_side.jpg' width=150 height=200>

        - **`json_front_file`**: 3d depth data of frpmt body json file

        - **`json_side_file`**: 3d depth data of side body json file

    - **Response**: Image file included length imformation
        - **Example**:

            <img src='http://203.252.147.202:8000/static/sweet_suit/upper.png' width=300 height=400>
